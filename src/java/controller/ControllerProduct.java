    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (action.equals("Search")) {
                ...
                RequestDispatcher rd = request.getRequestDispatcher("codes.jsp");
                rd.forward(request, response);
            } else if (action.equals("Delete")) {
                String code = request.getParameter("txtCode");
                Products sp = new Products();
                sp.delete(code);
                String url = "ControllerProduct?action=Search&txtTenSP=";
                RequestDispatcher rd = request.getRequestDispatcher("edditpro.jsp");
                rd.forward(request, response);
            } else if (action.equals("New")) {
                String url = "codes.jsp";
                ...
                int discu = 20;
                Products sp = new Products();
                sp.insert(code, name, fpri, des, imp, cata, discu, tage);
                RequestDispatcher rd = request.getRequestDispatcher("codes.jsp");
                rd.forward(request, response);
            } else if (action.equals("Add New Product")) {
                RequestDispatcher rd = request.getRequestDispatcher("codes.jsp");
                rd.forward(request, response);
            } else if (action.equals("Mens")) {
                RequestDispatcher rd = request.getRequestDispatcher("mens.jsp");
                rd.forward(request, response);
            }else if(action.equals("Womens")){
